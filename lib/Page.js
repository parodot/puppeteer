...
class Page extends EventEmitter {
  ...
  async exposeFunction(name, puppeteerFunction) {
    ...
    function addPageBinding(bindingName) {
      console.log('addPageBinding %s', bindingName);
      window[bindingName] = async(...args) => {
        console.log('Bound %s', bindingName);
        ...
        console.log('callbacks size ', me['callbacks'].size);
  ...
  async _onConsoleAPI(event) {
    if (event.type === 'debug' && event.args.length && event.args[0].value === 'driver:page-binding') {
      ...
      console.log('NowError 2 %s, %s', name, this.url());
      // console.log(event);
      // console.log(this._pageBindings);
      // console.log(this);
      if (typeof this._pageBindings[name] !== 'function'){
        this._bindingInvocationErrors[name] = this._bindingInvocationErrors[name] ? this._bindingInvocationErrors[name] : 0;
        this._bindingInvocationErrors[name]++;
        console.log(this._bindingInvocationErrors);
        return;
      }
      const result = await this._pageBindings[name](...args);
      console.log('NowError 3 %s', name);
